...
import com.shiroha.mmdskin.render.material.PmxMaterialLoader;
...
public class GpuSkinningModelInstance extends BaseModelInstance {
    ...
    public static GpuSkinningModelInstance create(NativeRenderBackendPort nativeBackend,
        ...
        GpuSkinningModelInstance result = createFromHandle(nativeBackend, model, modelDir, isPMD ? null : modelFilename);
        if (result == null) {
    ...
    public static GpuSkinningModelInstance createFromHandle(NativeRenderBackendPort nativeBackend,
                                                            long model,
                                                            String modelDir) {
        return createFromHandle(nativeBackend, model, modelDir, null);
    }

    private static GpuSkinningModelInstance createFromHandle(NativeRenderBackendPort nativeBackend,
                                                              long model,
                                                              String modelDir,
                                                              String modelFilename) {
        if (!ensureComputeShaderInitialized()) {
    ...
    float morphedMaterialValue(int materialIndex, int component, float baseValue) {
        return getMorphedMaterialValue(materialIndex, component, baseValue);
    }
