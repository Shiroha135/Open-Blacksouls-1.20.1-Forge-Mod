...
public abstract class BaseModelInstance implements ModelInstance {
    ...
    protected float getEffectiveMaterialAlpha(int materialIndex, float baseAlpha) {
        return getMorphedMaterialValue(materialIndex, MATERIAL_MORPH_MUL_ALPHA_OFFSET, baseAlpha);
    }

    protected float getMorphedMaterialValue(int materialIndex, int component, float baseValue) {
        if (materialMorphResultsByteBuffer == null
                || materialIndex < 0
                || materialIndex >= materialMorphResultCount
                || component < 0
                || component >= 28) {
            return baseValue;
        }
        int mulOffset = materialIndex * MATERIAL_MORPH_STRIDE_FLOATS + component;
        int addOffset = materialIndex * MATERIAL_MORPH_STRIDE_FLOATS + 28 + component;
        int capacity = materialMorphResultsByteBuffer.capacity() / 4;
        float multiplier = mulOffset < capacity ? materialMorphResultsByteBuffer.getFloat(mulOffset * 4) : 1.0f;
        float addition = addOffset < capacity ? materialMorphResultsByteBuffer.getFloat(addOffset * 4) : 0.0f;
