...
final class OpenGlModelFactory {
    ...
    static OpenGlModelInstance create(NativeRenderBackendPort nativeBackend,
        ...
        OpenGlModelInstance result = createFromHandle(nativeBackend, model, modelDir, isPMD ? null : modelFilename);
        if (result == null) {
    ...
    private static OpenGlModelInstance createFromHandle(NativeRenderBackendPort nativeBackend,
                                                         long model,
                                                         String modelDir,
                                                         String modelFilename) {
        ensureShaderInitialized();
        ...
        try {
            ...
            if (!texKeys.isEmpty()) TextureRepository.releaseAll(texKeys);
            return null;
        }
    }
