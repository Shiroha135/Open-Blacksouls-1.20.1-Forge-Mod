...
public class SkillAuraBlade extends WeaponSkill {

    @Override
    public String getSkillId() { return "bs2_skill_aura_blade"; }
    ...
    @Override
    public String getTranslationKey() { return "skill.blacksouls.aura_blade.name"; }
    ...
    protected boolean isWeaponEquipped(Player player) {
        ...
        return !mainHand.isEmpty() && mainHand.getItem() == BlackSouls.ANDOR_SWORD.get();
    }
    ...
    public void execute(ServerPlayer player, BSPlayerStats stats) {
        ...
        player.sendSystemMessage(Component.translatable("message.blacksouls.skill.aura_blade.use", player.getName().getString()).withStyle(ChatFormatting.WHITE));
        ...
        server.tell(new net.minecraft.server.TickTask(Math.max(1, (int) Math.round(198 / 50.0)), () -> {
            if (!target.isRemoved()) {
                ...
                playSkillSound(target, BlackSouls.FLASH1_EVENT.get(), 1.0f, 1.0f);
                playSkillSound(target, BlackSouls.SAINT9_EVENT.get(), 1.0f, 1.0f);
            }
        }));
    }
    ...
    private void applyHit(ServerPlayer player, LivingEntity target, BSPlayerStats stats, boolean isSureHit) {
        ...
        double rawDamage = (stats.attack * 4.0 + stats.bonusMatk * 3.0) - (bDef * 2.0 + bMdf * 2.0);
