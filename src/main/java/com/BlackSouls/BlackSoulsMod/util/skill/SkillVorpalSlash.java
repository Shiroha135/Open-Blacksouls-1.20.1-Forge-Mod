...
public class SkillVorpalSlash extends WeaponSkill {

    @Override
    public String getSkillId() { return "bs2_skill_vorpal_slash"; }
    ...
    @Override
    public String getTranslationKey() { return "skill.blacksouls.bs2_skill_vorpal_slash.name"; }
    ...
    protected boolean isWeaponEquipped(Player player) {
        ...
        return !mainHand.isEmpty() && mainHand.getItem() == BlackSouls.VORPAL_BLADE.get();
    }
    ...
    public void execute(ServerPlayer player, BSPlayerStats stats) {
        ...
        int comboStage = SkillUtils.getVorpalComboStage(player);
        player.swing(InteractionHand.MAIN_HAND, true);

        player.sendSystemMessage(Component.translatable("message.blacksouls.skill.vorpal_slash.use", player.getName().getString()).withStyle(ChatFormatting.WHITE));
        ...
        switch (comboStage) {
            case 0 -> {
                ...
                server.tell(new net.minecraft.server.TickTask(0, () -> {
                    if (!target.isRemoved()) {
                        applyHit(player, target, stats, 4.0, false);
                        playSkillSound(target, BlackSouls.SLASH2_EVENT.get(), 1.0f, 1.0f);
                    }
                }));
                server.tell(new net.minecraft.server.TickTask(Math.max(1, (int) Math.round(400 / 50.0)), () -> {
                    if (!target.isRemoved()) {
                        applyHit(player, target, stats, 4.0, false);
                ...
                SkillUtils.setVorpalComboStage(player, 1);
            }
            case 1 -> {
                playAnim(target, 244);
                ...
                playSkillSound(target, BlackSouls.SLASH1_EVENT.get(), 1.0f, 1.0f);
                playSkillSound(target, BlackSouls.SLASH2_EVENT.get(), 1.0f, 1.0f);
                playSkillSound(target, BlackSouls.SLASH3_EVENT.get(), 1.0f, 1.0f);

                SkillUtils.setVorpalComboStage(player, 2);
            }
            case 2 -> {
                playAnim(target, 245);
                ...
                server.tell(new net.minecraft.server.TickTask(0, () -> {
                    if (!target.isRemoved()) {
                        applyHit(player, target, stats, 3.0, false);
                        playSkillSound(target, BlackSouls.SLASH1_EVENT.get(), 1.0f, 1.0f);
                    }
                }));
                server.tell(new net.minecraft.server.TickTask(Math.max(1, (int) Math.round(400 / 50.0)), () -> {
                ...
                server.tell(new net.minecraft.server.TickTask(Math.max(1, (int) Math.round((400 + 400) / 50.0)), () -> {
                    if (!target.isRemoved()) {
                        applyHit(player, target, stats, 3.0, false);
                        playSkillSound(target, BlackSouls.SWORD3_EVENT.get(), 1.0f, 1.0f);
                    }
                }));
                server.tell(new net.minecraft.server.TickTask(Math.max(1, (int) Math.round((400 + 400 + 460) / 50.0)), () -> {
                    if (!target.isRemoved()) {
                        applyHit(player, target, stats, 6.0, false);
                        playSkillSound(target, BlackSouls.SLASH9_EVENT.get(), 1.0f, 1.0f);
                    }
                }));

                SkillUtils.setVorpalComboStage(player, 0);
            }
        }
    }
