...
import com.BlackSouls.BlackSoulsMod.mixin.client.GameRendererAccessor;
...
public final class HeldItemOutlineRenderer {
    ...
    private static final boolean ENABLE_COMPAT_DEBUG_LOG = Boolean.getBoolean("blacksouls.debugHeldOutlineCompat");
    ...
    @Nullable
    public static HandEffectTarget getRenderableHand(LocalPlayer player, InteractionHand hand) {
        ...
        try {
            for (int index = 0; index < targets.size(); index++) {
                ...
                if (!beginEmbeddiumCompatCapture(minecraft, minecraft.getMainRenderTarget(), target.hand(),
                    ...
                    restoreProjectionMatrix(originalProjection, originalVertexSorting);
                    poseStack.popPose();
                    continue;
                }
    ...
    private static ResolvedRenderState resolveCachedRenderState(CaptureState state, boolean handEnabled, ItemStack observedStack, ItemStack liveStack) {
        if (state.cachedObservedHandEnabled == handEnabled
    ...
    private static void inspectMaskCoverage(Minecraft minecraft, CaptureState state) {
        if (!ENABLE_MASK_DEBUG_READBACK || !LOGGER.isDebugEnabled() || state.outlineMaskTarget == null || minecraft.level == null) {
            return;
        }
        ...
        try (NativeImage image = new NativeImage(NativeImage.Format.RGBA, state.outlineMaskTarget.width, state.outlineMaskTarget.height, false)) {
            ...
            LOGGER.debug("[HeldItemOutline] Hand={} mask sample coverage={} active={}/{} centerAlpha={} corners=[{},{},{},{}] bounds={}..{},{}..{}",
                    state.hand,
            ...
            LOGGER.warn("[HeldItemOutline] Failed to inspect outline mask texture", exception);
        } finally {
    ...
    private static void debugLog(Minecraft minecraft, String message) {
        ...
        LOGGER.debug("[HeldItemOutline] {}", message);
    }
    ...
    private static void restoreProjectionMatrix(Matrix4f projectionMatrix, VertexSorting vertexSorting) {
    ...
    private static final class CaptureState {
        ...
        void closeTarget() {
            if (this.outlineMaskTarget != null) {
                this.outlineMaskTarget.destroyBuffers();
                this.outlineMaskTarget = null;
            }
            this.restoreTarget = null;
            this.captureActive = false;
            this.capturedThisFrame = false;
    ...
    public static void closeTargets() {
        if (bloomBlurTargetA != null) {
            bloomBlurTargetA.destroyBuffers();
            bloomBlurTargetA = null;
        }
        if (bloomBlurTargetB != null) {
            bloomBlurTargetB.destroyBuffers();
            bloomBlurTargetB = null;
        }
        if (bloomBlurNearTarget != null) {
            bloomBlurNearTarget.destroyBuffers();
            bloomBlurNearTarget = null;
        }
        MAIN_HAND_STATE.closeTarget();
        OFF_HAND_STATE.closeTarget();
        embeddiumCaptureRenderBuffers = null;
        activeCaptureState = null;
        activeHandProjectionMatrix = null;
    }
}
