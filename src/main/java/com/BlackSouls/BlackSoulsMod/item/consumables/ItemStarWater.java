...
public class ItemStarWater extends Item {

    public ItemStarWater(Properties properties) {
    ...
    public @NotNull InteractionResultHolder<ItemStack> use(Level level, Player player, @NotNull InteractionHand hand) {
        ...
        if (!level.isClientSide() && player instanceof ServerPlayer serverPlayer) {
            ...
            float damageToDeal = currentHp * 0.9F;
            player.hurt(player.damageSources().magic(), damageToDeal);
    ...
    public void appendHoverText(@NotNull ItemStack stack, @Nullable Level level, List<Component> tooltip, @NotNull TooltipFlag flagIn) {
        tooltip.add(Component.translatable("item.blacksouls.star_water.lore.1").withStyle(ChatFormatting.WHITE));
        tooltip.add(Component.translatable("item.blacksouls.star_water.lore.2").withStyle(ChatFormatting.GRAY, ChatFormatting.ITALIC));
    }
}