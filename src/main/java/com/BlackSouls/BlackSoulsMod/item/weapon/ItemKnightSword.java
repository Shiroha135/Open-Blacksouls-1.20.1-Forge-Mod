...
public class ItemKnightSword extends ItemBSWeaponBase {

    public ItemKnightSword(Properties properties) {
    ...
    public boolean hurtEnemy(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        if (!attacker.level().isClientSide() && attacker instanceof ServerPlayer player) {
            ...
            PacketPlayAnim animPacket = new PacketPlayAnim(125, target.getX(), target.getY() + target.getBbHeight() / 2.0F, target.getZ());
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag flagIn) {
        tooltip.add(Component.translatable("item.blacksouls.knight_sword.lore.1")
                .withStyle(ChatFormatting.GRAY));

        tooltip.add(Component.translatable("item.blacksouls.knight_sword.lore.2")
