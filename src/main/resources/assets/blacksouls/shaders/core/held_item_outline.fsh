...
vec3 outlineBaseColor(vec2 uv) {
    if (ColorMode < 0.5) {
        ...
        return mix(OutlineColor.rgb, vec3(1.0, 0.96, 0.58), glint * 0.72);
    }
    ...
    if (ColorMode < 1.5) {
        ...
        return mix(color, vec3(1.0, 0.98, 0.64), glint * 0.78);
    }
    if (ColorMode < 2.5) {
        float hue = fract(uv.x * 0.22 + uv.y * 0.14 - Time * 0.08 * ColorScrollSpeed);
        float glint = pow(max(0.0, sin((uv.x * 28.0 - uv.y * 20.0) + Time * 8.5 * max(ColorScrollSpeed, 0.6))), 16.0);
        return mix(hsv2rgb(vec3(hue, 0.85, 1.0)), vec3(1.0, 0.98, 0.64), glint * 0.58);
    }
    float glint = pow(max(0.0, sin((uv.x * 28.0 - uv.y * 20.0) + Time * 8.5 * max(ColorScrollSpeed, 0.6))), 16.0);
    return mix(sampledScrollColor(uv), vec3(1.0, 0.98, 0.64), glint * 0.58);
}

void main() {
    vec2 texel = 1.0 / max(ScreenSize, vec2(1.0));
    float radius = max(OutlineWidth, 0.35);
    ...
    shell *= 0.82 + flowLine * 0.42;

    float glow = shell * shell * (0.22 + GlowStrength * 0.68);
    ...
    color *= 1.0 + GlowStrength * 0.22 + depthContrast * DepthWeight * 0.10 + flowLine * 0.42;
